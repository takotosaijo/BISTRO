...
1. **F19 最小前端试验台**：单文件 HTML + 章回滑块 + 对话，由 FastAPI 托管，不进构建工具。
   目的是让「拨时间线，人变了」这件事可以用手拨出来给人看——现在只能靠 `/docs` 和 curl 感受。
   排在 F21 扩面前面：探针网已经覆盖了三类主要失败模式（出戏 / 剧透 / 口吻串位），
   再扩到 12 个角色是边际收益递减；而「没有人能用眼睛和手感受这个产品」是更大的缺口。
2. **F21 扩面**：框架与判据已跑通（3 角色 / 13 条，13/13 通过），把探针补到 12 个角色、
   每角色 10 条以上；补的时候记得「探针里不写被禁的词」这条硬规矩。
