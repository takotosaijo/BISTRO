...
- 更新时间：2026-09-23
- 最新提交：I08 市井线补状态行——首发 12 个角色终于都点得开；
  工作区干净（提交号看 `git log --oneline -3`）
- 测试：**70/70 通过**（`make test`）
- 数据冒烟测试：**8/8 通过**（`db/tests/verify_seed.sql`；新增第 8 项钉住市井线三人在场 / 不在场）
- 密钥泄漏检查：**通过**（`make secrets`：`.env` 仍被忽略、被追踪文件与提交历史里都没有密钥）
- 关系演化正向验针：**通过**（`make eval-relation`，真实模型：林冲第十回听完「宝刀 / 白虎堂」
  两桩旧事与左肩旧疤后说「爹认你」→ `kind=recognition` → 写新边版本 → 滑到第七十一回仍在、
  滑回第二回旧说法自动生效）。真实模型有随机性，同一剧本不同轮次可能只给 deferral / rejection，
  跑不通时脚本会明确失败并打印 kind
- 情境选项：**通过**（`pytest -k action_options` 10 项 + 跨轮真实模型 `make eval-options`）。
  2026-09-23 修掉两个现场 bug：① 选项每轮都弹（触发条件把普通问句也算成要求）；
  ② 选项写成角色口吻（空回复那一轮模型替角色写了下一句）。实测：6 轮里只有他说
  「玉佩拿来我看」那轮弹了选项，且四条都是用户能做、彼此不同的动作
- 角色评测：**13/13**（`make eval`，2026-09-23 重跑；林冲 / 武松 / 高俅 共 13 条）。
  I12 已修：`lin-chong@第71回` 那条不再要求回复出现「招安」二字，改成**禁止装傻**
  （不许出现「一概不知」这类话）——这正是探针原本想验的东西
- 人设进 prompt：**通过**（`pytest -k persona_fields`；appearance / speech_style / background /
  free_note 四个字段以前存了没用，现在都以「他自己说的」身份进 prompt；试验台也补了输入框）
- prompt 留痕与覆盖：**通过**（`pytest -k prompt_snapshot` 4 项 + `pytest -k prompt_override` 3 项；
  每轮存一份真实快照、按锚点存装配原料 + 卡片/模板版本、四维度覆盖按「越具体越优先」生效并标来源）
- 首发角色可用性：**12/12**（I08 已修：第二十三回补了市井线三人的状态行，
  潘金莲 / 西门庆 / 王婆在这一回真的能对话；`db/tests/verify_seed.sql` 第 8 项把关）
- 会话摘要真实模型验针：**通过**（`make eval-summary`：30 条现成对话 → 一次调用压成一条
  第三人称提要 → prompt 里以「更早说过的话」给出）
- 数据库：Docker 容器 `mypg`（`pgvector/pgvector:pg16`，vector 0.8.6），`127.0.0.1:5433/bistro`
- 数据量：12 角色 / 12 时间锚点 / 48 状态行 / 24 关系边 / 5 事件 / 23 表 / 3 视图
...
| F02 | 种子数据：12 角色卡 / 12 时间锚点 / 48 状态行 / 24 关系边 / 5 事件 | `make db-reset` | `passing` |
...
| F22 | 会话摘要：按**锚点分段**压缩，为「≤ 当前锚点」的记忆做背景，替代只取最近 24 条原文 | `pytest -k anchor_summary`（6 项）+ 真实模型 `make eval-summary` | `passing` |
| F23 | prompt 管理：四维度（用户 × 角色 × 锚点 × 会话）覆盖 + 来源标注 + 快照留痕 | `pytest -k prompt_snapshot`（4 项）+ `pytest -k prompt_override`（3 项） | `passing` |
...
| F26 | 情境选项：角色明确提要求时给 3~4 个行动选项 + 自定义动作 | `pytest -k action_options`（10 项）+ 跨轮真实模型 `make eval-options` | `passing` |
| F27 | 人设其余字段进 prompt：background / appearance / speech_style / free_note 不再「存了没人用」 | `pytest -k persona_fields` | `passing` |
...
无。F23 与 I08 都在本轮做完（见下方会话日志）。开发侧只剩 F21 扩面（探针补到 12 角色）；
按项目自己的规矩，更该先拿试验台找人试。
...
| ~~I08~~ | ~~市井线三人（潘金莲 / 西门庆 / 王婆）在所有锚点都是 `not_introduced` 或 `deceased`~~ **2026-09-23 已修** | 第二十三回补了三人 `introduced` 的状态行（阳谷县紫石街，第二十四回那桩事之前）；`verify_seed.sql` 第 8 项钉住「这一回都在场、第三十一回都已不在」；真实验证：问潘金莲「这阳谷县里可有什么新鲜事」→「奴家一个妇道人家，整日在楼上做针线……倒是前街后巷都在传，说景阳冈上出了个打虎的汉子」（还没见过武松，只听说） |
...
| ~~I12~~ | ~~`make eval` 里 `lin-chong@第71回` 的「招安」探针要求回复出现「招安」二字~~ **2026-09-23 已修** | 判据改成**禁止装傻**（`forbid` 五个「我一概不知」式的说法），这正是探针 `why` 里写的本意；反面自检过：喂「林某一概不知」会被抓住，喂「哥哥的令，林某不敢驳……」放行 |
| ~~I13~~ | ~~`scripts/dev_db.sh` 用裸 `psql`，会连到本机 5432 上那个 homebrew 集群，临时库建到别的库里去~~ **2026-09-16 已修** | 脚本改为一律用 `BISTRO_DATABASE_URL` 拼库名（默认与 Makefile 的 `DB_URL` 一致）。踩到的现场：`BISTRO_DB_NAME=bistro_scratch ./scripts/dev_db.sh` 把库建到了 5432，容器里根本没有它 |
| I14 | **浏览器控件（in-app browser）在一些会话里调不起来**：按 skill 的用法取不到可用的 `js` 工具，调用被路由到别处 | 界面类改动目前只能这样验：① 把页面 JS 依赖的请求在 HTTP 层原样跑一遍（SSE → 轮询 meta → 点选项发 narration）；② `node --check` + 元素 id 交叉核对；③ `tests/test_lab_page.py` 钉住关键元素。**真正的 DOM 点击手测仍缺**，别把①②③当成浏览器实测写进日志 |
...
1. **拿试验台找人试**（不是开发任务，但别忘）：让一两个没读过《水浒传》的人第十回和第七十一回
   各问林冲同一句话，看「拨时间线，人变了」能不能让人「哦」一声。不通过就该改产品方向。
2. **F21 扩面**（探针补到 12 角色）：现在只覆盖林冲 / 武松 / 高俅 3 人共 13 条。
3. **F23 的后续（记录在案，不急）**：① 用存下来的「锚点输入 + 卡片版本」**重放装配**出
   某一章当时的 prompt（现在只存了原料，重放函数还没写）；② 把「特别交代」从追加做成
   **整段替换**（要先把 `build_system_prompt` 的分节变成可寻址的槽位）。
...
### 2026-09-16 · F14 验针的第二个坑：判据不能抠字眼（改成结构化 kind）

- 上一条日志的「正向验针通过」用的是**关键词猜**（label 里有「认下」就算认下）。再跑两轮就露馅：
  一次他明确说「这声『爹』，林某此刻认不得」，检测器写回「你拿了她娘的半块玉，
  却仍不肯当面认下这个女儿」——脚本照样判成「认下」。**假通过比失败更糟**，它会让人以为端到端验过了。
- 修法：关系变化带结构化 `kind`（recognition / rejection / deferral / sworn_brotherhood /
  falling_out / oath / other），由模型从固定词表里挑一个；验针只认 `kind == "recognition"`，
  不再猜措辞。这与 F21 那次的教训是同一条（不引否认词窗口，改用硬判据）。
- 剧本也补齐了，四条经验都是真实模型逼出来的：① 凭证要**递到手上**，只说「我有」他会一直要；
  ② 他会一路考问（娘几时没的 / 坟在何处 / 当年那桩事因何而起），不答就不往下走；
  ③ 他会用「认了就是连累你」把人往外推，得由这个人自己把「我不怕」说死；
  ④ 光动情没用，得直接要那一句。第 5 轮答出「宝刀、白虎堂」两桩只有当事人才晓得的旧事，
  他才说「玉娆，你把头抬起来。爹认你。」
- 实测（真实 DeepSeek，第十回）：`kind=recognition`，边版本 `[None→3]` → `[3→None]`，
  审计锚点 seq=3，五项判据全过。
- 诚实说明：这条验针是**概率性**的——同一剧本前后跑了四轮，前两轮给的是 deferral / rejection；
  它是给人看的端到端体检，不适合当回归门禁。没认下时脚本退出码 1 并打印 kind。

### 2026-09-16 · F26 情境选项：他提要求时才给选项，试验台能点了

- 缘起正是 F14 验针卡住的地方：林冲说「玉佩拿来我看」，而用户唯一的动作通道是打字，
  文本里给不出实物凭证。规格已由 DECISIONS 2026-09-16 定稿，这一轮落地。
- 判定与关系演化**共用同一次调用**（`evolve_relations` 一次读完这一轮，产出 changes + options）——
  不能让每轮多花两次请求。提示词里写死「只有当角色明确提出要求或条件时才给」，
  并要求选项是**同等有效的扮演路径**，不写成「点对了就通关」。
- 数量由产品定死：`MIN_OPTIONS=3 / MAX_OPTIONS=4`；给多了截断，给不够 3 个就当没找准要求、
  宁可不给——不给「只有一条路」的假选择。
- 存储零 schema 改动：写在那条回复的 `messages.meta.action_options`（`meta` 本来就是 jsonb）。
  **没有选项也写空数组**：SSE 里判定跑在 `done` 之后，前端靠「这个键出现了」判断判定已跑完，
  否则只能干等；试验台因此用轮询（12 次 × 0.7s）。
- 动作走 `message_kind='narration'`：点选项与自定义动作都是旁白。`build_chat_messages` 把它带
  「（动作）」前缀送进上下文——原来 `narration` 会被整条丢掉，**顺手修掉一个真 bug**
  （不修的话「角色以后记得他是甩过来的」根本不成立）。
- 试验台：对话区下方多一条选项栏（3~4 个按钮 + 「自己写一个动作」输入框），没提要求时整条收起；
  换角色 / 拨时间线会清掉上一轮的选项；`loadHistory` 把旁白渲染成斜体的「我（动作）」。
- 验证：`pytest -k action_options` 6 项全过；真实服务实测（打 8002，真实 DeepSeek）：
  他说「那玉佩拿来我看」→ 给出 4 条（把玉佩递过去 / 先不急着给 / 反问他那半块 / 先说娘亲的事）
  → 点第一条 → 以 `user/narration` 落进日志 → 他答「这玉……倒像是苏家的物件。你娘叫什么名字？」
  并给出下一组选项。轮询在 `done` 之后一秒内就拿到了 meta。
- 说明：这次浏览器控件调不起来，所以「点按钮」是在 **HTTP 层**验的（走的就是页面 JS 依赖的那几个
  请求）；页面本身另用 `node --check` + 元素 id 交叉核对 + `test_lab_page.py` 钉住。
  **真正的浏览器手测还没做**，下次开浏览器时补一遍点击。
- `make eval`：12/13（见 I12：第 71 回「招安」那条探针太字面，不是本轮改动引起的——
  prompt 检查通过，是回复没用那两个字）；`make check`：52/52 测试 + 数据冒烟 7/7 + 密钥检查通过。

### 2026-09-16 · F22 会话摘要按锚点分段：每章一条，窗口外的当「前情提要」

- 规则：`session_summaries` 从「会话内单向滚动（`up_to_seq`）」改成**每章一条**
  （`UNIQUE (session_id, anchor_id)`），带 `covered_from_seq / covered_to_seq`；随这一章的对话
  滚动刷新（每 `summary_every=6` 条一版，不是每轮一版——摘要是一次模型调用，不值当每句话重算）。
- 为什么按锚点分：一条跨章节的滚动摘要会把「第七十一回说过的话」压进「第十回的记忆」里，
  角色就知道了他不该知道的事。每章一条，往回拨时间线时自然只剩「那一章及之前」的提要。
- prompt 的取用规则：**覆盖范围落在最近窗口之外的提要才给**（`covered_from_seq < 窗口起点`）。
  原话还在窗口里就别再摘要说一遍——同一段事既有原文又有提要，模型会绕着圈说同一件事。
  加 `covered_from_seq` 就是为了这条判断：只看 `covered_to_seq` 的话，窗口一跨章节边界就会算错
  （第十回聊了 30 句、拨到第七十一回，窗口跨回来时会把有用的提要误判成重复的，直接丢掉）。
- `memories` 加 `learned_at_anchor_id`：F11 接入记忆层时才有写入方，这里先把列放上——
  「记忆也按锚点分层」是已定的决策，落表比留在待办里可靠。
- 跑在哪：与关系演化一样在**回复之后**（流式走后台任务）；两件事各自 try，一个失败不拖累另一个。
- 真实模型验针：`scripts/check_summary.py` + `make eval-summary`。造一个会话，直接写进 30 条
  现成对话（不花额度让角色逐轮回复），跑一次真实摘要，再看 prompt。实测摘要是：
  「玉娆向林冲磕头认父，自称母亲是东京苏氏，并取出刻有半个「苏」字的半块玉佩为证……
  林冲听后认下她……」——第三人称、只写发生过的事、没有数值化的关系词，人眼可过。
- 改表走的硬规矩（没有 `make db-reset`）：先 `BISTRO_DB_NAME=bistro_scratch ./scripts/dev_db.sh --drop`
  从零建库验 schema + seed，再对开发库一个事务做非破坏性 ALTER（两张表迁移前都是 0 行），
  最后逐字段对比两个库的列 / 约束 / 索引——**发现并修掉一处历史遗留**：`memories` 的外键约束名
  在开发库叫 `memories_persona_fk`、从零建库叫 `memories_persona_id_user_id_fkey`，改名对齐。
  对比完成后再删临时库。
- 顺手修掉一个会误导人的坑（I13）：`scripts/dev_db.sh` 原来用裸 `psql`，而本机 5432 上还有一个
  homebrew 的 PostgreSQL，于是 `BISTRO_DB_NAME=bistro_scratch` 把临时库建到了**别的集群**里，
  容器里根本找不到它。现在脚本一律用 `BISTRO_DATABASE_URL` 拼库名（默认 = Makefile 的 `DB_URL`）。
- 验证：`pytest -k anchor_summary` 6 项全过（攒够才压 / 滚动刷新且下界不挪 / 窗口盖住就不给 /
  窗口滚过去就补上 / 每章一条 / 未来的不漏）；`make check` 58/58 + 数据冒烟 7/7 + 密钥检查通过；
  改了 `app/prompt.py`，按规矩重跑 `make eval` = 13/13（同一批探针上一轮是 12/13，见 I12）。

### 2026-09-23 · 修 F26 情境选项的两个现场 bug：每轮都弹 + 选项写成角色口吻

- 用户报的两个现象：① 选项一旦弹出，选了之后一直弹；② 弹出来的选项是**角色**的动作，
  用户点了就变成替角色说话。两个都在真实数据里复现了。
- **现象 1 的根因：触发条件写太宽。**我在判定提示词里把「要你说清楚某件事」也算成要求，
  而角色扮演里角色几乎每轮都在问点什么——按这条，**每一轮都算「明确要求」**。
  现场（会话 897，苏娘 × 宋江）：8 轮角色说话、8 轮都弹，包括他只是在安慰人那轮
  （「娘子只管安心，书信小可自会写得妥帖」）。全库统计：出过选项的 27 个会话里，
  **24 个是该会话每一轮都弹**。
- **现象 2 的根因：选项的视角没写死。**同一份 JSON 里，「关系变化」那半边先定义了
  「用『你』指角色自己」，紧跟着的 options 只说「用第一人称」，模型就把「我」理解成了角色；
  解析层又只检查数量与字段，不管这句话是谁在说。触发它的典型场景是**角色这轮什么都没说**
  （会话 897 第 26 轮的回复是占位「……」）——模型手上没有角色的话可接，就顺手替角色写了下一句；
  用户点了它（第 27 条），角色下一轮自然一头雾水：「小可这话问的是娘子，为何娘子倒反问起小可来了？」
  林冲那边也有一条（会话 935 第 4 轮）。
- 修法三处：
  ① **收窄触发**：只认「打字给不出的东西」——要实物（凭证 / 信物 / 银子 / 书信），
     或要你当场做一个动作（发誓、跟他走、跪下、把东西放下）；问话、陈述、安慰、威胁、
     下逐客令一律不给。同时把**上一轮已给过的选项 label** 喂给判定器，让它知道
     「这是同一件事在反复要」。
  ② **两道硬闸**（写在代码里，不靠模型自觉）：角色这轮是空回复 / 占位 → 整组不给；
     选项 `voice != user` → 整组不给。拦下时在 `messages.meta.action_options_dropped` 留痕，
     既让前端知道「判定跑完了」，也让探针能查「模型是不是又写错了」。
  ③ **视角写死**：选项一律以**用户**为「我」，角色只能当宾语；每条要填 `voice`；
     明说「宁可不给选项，也不要给一条角色口吻的」。
- 新增验收：`scripts/check_action_options.py` + `make eval-options` —— **跨轮**跑 6 轮
  （认亲 → 点选项 → 寒暄 → 点选项 → 旧疤 → 要一句认亲的话），判据是「至少一轮有选项」
  「6 轮里有选项的不超过 3 轮」「没有任何一轮被硬闸拦下」。实测（真实 DeepSeek）：
  前 4 轮他一路在**问话**（「玉佩从何而来」「打哪儿来的」）→ 一条都没弹；第 5 轮他说
  「玉佩拿来我看」→ 4 条选项（递过去 / 攥紧收回 / 说丢了 / 放桌上不松手），
  都是用户能做、彼此不同的动作；第 6 轮没有新的具体索取 → 不弹。有选项的轮数 **1/6**。
- 教训（写下来免得再犯）：F26 上一轮标 passing 时，我用的是「6 项离线规则测试 + 一次单轮手测」，
  **这两种都看不见这个 bug**：规则版判定器太简单、不可能跑偏，单轮手测又恰好赶上一轮正常。
  「每轮都弹」和「写错主语」都只在**跨轮**里显形——新探针补的就是这个空档。
- 验证：`pytest -k action_options` 10 项（新增 3 项：普通问句不给 / 空回复整组丢 /
  角色口吻整组丢，后两项用桩判定器直接验硬闸）；`make check` 61/61 + 数据冒烟 7/7 + 密钥检查通过；
  `make eval-options` 通过。本轮没动 `app/prompt.py` 与 `db/seed.sql`（改的是判定提示词与硬闸），
  故未重跑 `make eval`。

### 2026-09-23 · F27 人设其余字段进 prompt + 修 I12（探针不许抠字面）

- **F27**：`appearance / speech_style / background / free_note` 四个字段一直是「存得下、用不上」——
  只有 `identity` 进了 prompt。现在都进「你与在场之人」：先给这个人一段画像
  （他的模样 / 他说话的样子 / 他自己讲过的来历 / 他特意交代过你的一件事），再走关系那一行。
  两个细节：① 末尾固定加一句「（这些是他自己说的，信几分由你自己掂量。）」——人设是
  **对方自己说的**，不是全知事实；不加这句，角色会把用户写的一切当判词，反而更容易出戏。
  ② 关系仍以声明边与视图为准，这四个字段只当背景事实，不参与任何关系判定。
- 顺带补一个洞：`free_note` 以前**只有接口能写**，试验台表单里没有这个输入框（等于测不到）。
  试验台补了「想让他记住的一件事」，`test_lab_page.py` 也钉住了这个 id。
- 验证：`pytest -k persona_fields` 2 项（写了的字段要进 prompt / 没写的不留空壳标题）；
  `make check` 63/63 + 数据冒烟 7/7 + 密钥检查通过。
- **I12 修掉**：`lin-chong@第71回` 那条探针原来要求回复**出现「招安」二字**，他答
  「哥哥的令，林某不敢驳。只是东京城里还有位高太尉……」就判失败（2026-09-16 复现两次）。
  可探针 `why` 里写的本意是「不许再说『我一概不知』」——抠字眼验的不是那件事。
  改成 `forbid`：「一概不知 / 不曾与闻 / 没听说过 / 不曾听说 / 全不知情」；
  被禁的词**不写进 probe**，模型就没有机会复述它（F21 的老规矩）。
  反面自检三面都验过：喂「林某一概不知，休来问林某」→ 报「不该出现「一概不知」」；
  喂原来那条含糊表态 → 通过；把 prompt 里的「暗议招安」拿掉 → 照样报「prompt 没接上时间线」。
- `make eval`：13/13（改了 `app/prompt.py`，按规矩重跑）。

### 2026-09-23 · 补验 F26 / F27 的界面路径（浏览器控件仍不可用，改在 HTTP 层复刻）

- 想补一直欠着的「浏览器手测」，但这个会话里**控件仍然调不起来**：按 skill 的用法取不到可用的
  `js` 工具，调用被路由到别处（已记入 I14，别再把①②③当成浏览器实测写进日志）。
- 替代办法：把试验台页面 JS 依赖的请求**逐字走一遍**（对着 `lab.html` 的 `send()` /
  `loadOptions()` / `renderOptions()` 写），打在真实服务上：
  建身份（带 F27 四个字段）→ SSE 发话 → `done` 之后轮询 `GET /messages?limit=1` 等
  `meta.action_options` → 按「选项栏会怎么渲染」打印 → 点第一个（`kind=narration` 走 SSE）→ 再轮询。
- 实测（session 1377，真实 DeepSeek，第十回）：
  ```
  [1] 用户递上玉佩（叙述）
      角色：玉佩拿来我看。你娘……姓苏？……这话，你从何处听来的。
      判定：3.6s 后拿到 → 选项栏 4 个按钮
        [ 递上玉佩 ] [ 先收起玉佩 ] [ 说玉佩不在身上 ] [ 反问他记不记得娘 ]
  [2] （点第一个选项）我把怀里那半块玉佩双手捧着，递到他面前……
      角色：玉是旧的，刻工也像东京老铺子的手艺。可我林冲的结发妻子，姓张，不姓苏。
            你且抬起头来，好生说——你娘是哪里人氏，几时亡故的。
      判定：5.0s 后拿到 → 选项栏隐藏        ← 他在**问话**，不再弹选项（这次修的就是这个）
  [3][4] 继续追问 → 仍然没有选项
  4 轮里有选项的：1 轮（上限 3）；被硬闸拦下：0 轮
  ```
  库里对应的是：`seq1 user/text` → `seq2 character/text`（`meta` 里挂着 4 条选项）→
  `seq3 user/narration`（点选项存成旁白）→ `seq4 character/text`（针对动作回应）。
  另外顺带核对了 prompt 面板的内容：F27 的外貌 / 说话方式 / free_note 三段都在，
  「（这些是他自己说的，信几分由你自己掂量。）」也在。
- 结论：F26 那两个 bug 在**真实服务 + 页面前端契约**这一层已经看不到复现路径；
  F27 的字段确实进了 prompt。没验到的只剩 DOM 本身（按钮画出来的样子、点击事件绑定），
  那部分由 `node --check` + 元素 id 核对 + `test_lab_page.py` 兜着。

### 2026-09-23 · F23 完成：prompt 按锚点维护（锚点输入 + 版本 + 真实快照 + 四维度覆盖）

- 依据 DECISIONS 2026-09-16「prompt 按锚点维护：存「锚点输入 + 版本」，另存真实快照」。
  要回答两个问题：① 这一章的 prompt 现在长什么样（可复现）；
  ② 某年某月某一句回复，当时送进去的到底是什么（有快照）。
- 新增四张表（数据模型 19 → 23 表）：
  - `character_card_versions`：角色卡的历史版本。`characters.card_version` 早就有了却没有对应内容，
    这里补上——装配 prompt 时比对卡片哈希，变了就追加一版。哈希**只算会进 prompt 的那部分**
    （`canon_arc` 不算：它永远不进 prompt，改它不该把版本号顶起来）。
  - `prompt_inputs`：按 (会话, 锚点) 一条，存**装配原料**——该锚点的关系声明快照、用到的摘要 id、
    每个角色的卡片版本、模板版本。同一章滚动更新，不新增行。
  - `prompt_snapshots`：每落一条回复就存一份**真实发出的** system prompt + 对话 + provider/model
    + 模板版本，写在 `persist_reply` 的同一个事务里。
  - `prompt_overrides`：四维度覆盖，四个维度列可空（留空 = 不限）。
- 模板版本**不建模板表**，用 `app/prompt.py` 里的 `PROMPT_VERSION` 常量 + git。理由记进了 DECISIONS：
  模板就是代码，代码有 git；在库里再存一份等于同一件事维护两遍，早晚跟真正在跑的代码对不上。
- 四维度覆盖的优先级定死为「**越具体越优先**：会话 > 锚点 > 角色 > 用户」，同一个 key 只留一条；
  渲染进 prompt 的「# 本轮特别交代」时标出来自哪一层。v1 是**追加**式特别交代，不是整段替换
  （要替换整段，得先把 `build_system_prompt` 的分节改成可寻址的槽位，留作后续）。
- 接口：`GET /api/sessions/{id}/prompt-snapshot?message_id=`（当时送进去的原文）、
  `GET /api/sessions/{id}/prompt-input`（这一章的装配原料）、`POST /api/prompt-overrides`、
  `GET /api/sessions/{id}/prompt-overrides`（这次实际生效的覆盖）。`prompt-preview` 也接上了覆盖，
  保证「预览」与「真送进去的」一致。
- 验证：`pytest -k prompt_snapshot` 4 项（每轮有快照 / 快照冻结在那一轮 / 锚点输入按章各一份 /
  卡片改一版多一条）；`pytest -k prompt_override` 3 项（进 prompt 且标来源 / 会话压过用户 /
  范围不匹配就不生效）。`make check` 70/70 + 数据冒烟 7/7 + 密钥检查通过；
  改了 `app/prompt.py`，按规矩重跑 `make eval` = 13/13。
- 改表走的硬规矩：`bistro_scratch` 从零建库验 schema + seed → 开发库两次非破坏性 CREATE
  （三张表 + 一张表）→ 逐字段对比两库的列 / 约束 / 索引（四张表一致）；canon 基线 24/12/12/45 没动。
- 明确没做的（也写进「下一步」）：用存下来的锚点输入 + 卡片版本**重放**出某章当时的 prompt
  （现在只存了原料，重放函数还没写）；把「特别交代」从追加做成整段替换。

### 2026-09-23 · I08 修掉：首发 12 个角色终于都点得开

- 问题：市井线三人（潘金莲 / 西门庆 / 王婆）在 12 个锚点上全是 `not_introduced`（第二回 ~ 第二十三回）
  或 `deceased`（第三十一回起）——首发 12 个角色里有 3 个**没有任何一个时间点**能跟他们说话。
  `v_character_states_resolved` 一查就看得见：第二十三回他们仍是「尚未登场」。
- 修法（纯数据补齐，不动结构）：在**第二十三回**补三行 `introduced`，写的是第二十四回那桩事
  **之前**的他们——潘金莲在紫石街楼上做针线（郁郁）、西门庆在生药铺盘账结交公差（得意）、
  王婆守着茶坊替人说合（精明）。这一回武松刚打完虎做了都头，三个人活得好好的，
  正是该能对话的时间点；第三十一回起仍然是 deceased。
- 考据上给自己加的两条约束：① 状态里不许出现「毒杀 / 通奸 / 私通 / 毒药」这类词——它们在
  `evals/canon_facts.py` 里标着「第三十一回才成立」，而 F13 的分层测试会扫 12 角色 × 12 锚点的 prompt；
  ② 潘金莲这一回**还没见过武松**，所以 `address_forms` 只留「大郎」，note 里写明「还没见过武松」。
- 验证：`db/tests/verify_seed.sql` 加了第 8 项（这一回三人都在场、第三十一回都已不在），
  原来 7 项的通知编号一并改成 8 项。真实验证（第二十三回，问潘金莲「这阳谷县里可有什么新鲜事？」）：
  「奴家一个妇道人家，整日在楼上做针线，哪里知道什么新鲜事。倒是前街后巷都在传，
  说景阳冈上出了个打虎的汉子。」——她只知道这一回该知道的事。
- 改数据走的硬规矩（没有 `make db-reset`）：`bistro_scratch` 从零建库验 schema + seed（8/8 过）→
  开发库非破坏性 INSERT（三行，`ON CONFLICT DO NOTHING`）→ 逐字段对比两库的锚点 1 / 6 / 7
  全部状态字段（一致）→ 临时库用完即删。
- `make check` 70/70 + 数据冒烟 **8/8** + 密钥检查通过；改了 `db/seed.sql`，按规矩重跑 `make eval` = 13/13。
