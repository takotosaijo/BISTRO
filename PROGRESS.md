...
| F24 | 开发用 admin 用户：预置多个人设与会话，试验台可切换身份 | 未实现 | `not_started` |
...
1. **F24 admin 用户 + 试验台身份切换**（P0，最便宜）：`make admin` 建固定账号，预置几个人设
   （路人 / 情人 / 女儿）与若干会话；试验台把写死的 `lab-ui` 改成可切。后面每件事都要靠它用手验。
2. **F12 重定义：人设语义 → 结构化关系**：保存人设时解析一次并落库（可手改、手改优先），
   一次声明产出两个方向；prompt 以结构化结果为准、persona 原文作背景。这是「不再默认素不相识」的落地。
3. **F14 关系演化（锚点版）**：互动后按阈值推进 `stage`，往 `relationship_changes` 落记录
   （该表要加 `anchor_id`，现在只能靠 `message_id` 反查）。
4. **F22 摘要按锚点分段** + `memories.learned_at_anchor_id`：每章一条该章对话摘要，
   prompt 取「≤ 当前锚点」的那些。这两处要改表（现在没有锚点列）。
5. **F23 prompt 按锚点维护 + 四维度覆盖**：存储形态已定稿（锚点输入 + 卡片/模板版本 + 真实快照），
   接着做覆盖表、来源标注与留痕。
6. **persona 其余字段进 prompt**：background / appearance / speech_style / free_note 现在存了没用。
7. **F21 扩面**（探针补到 12 角色）、**I08 市井线补状态行**：不阻塞主线，可穿插做。
8. **拿试验台找人试**（不是开发任务，但别忘）：让一两个没读过《水浒传》的人在第十回和第七十一回
