...
- 更新时间：2026-09-23
- 最新提交：修 F26 情境选项的两个现场 bug（每轮都弹 / 选项写成角色口吻）；工作区干净
  （提交号看 `git log --oneline -3`）
- 测试：**61/61 通过**（`make test`；比上轮多 3 项＝F26 的回归用例）
...
- 角色评测：**13/13**（`make eval`，2026-09-16 改完 prompt 后重跑；林冲 / 武松 / 高俅 共 13 条）。
  同一批探针上两次是 12/13——`lin-chong@第71回` 那条要求回复出现「招安」二字，他那轮改说
  「哥哥的令，林某不敢驳」就判失败。这条是概率性的，**判据太字面的问题仍开着**，见 I12
...
| I12 | `make eval` 里 `lin-chong@第71回` 的「招安」探针要求回复**出现「招安」二字**；他答「哥哥的令，林某不敢驳……」就判失败（prompt 检查本身是通过的） | 2026-09-16 重跑复现两次。判据太字面，属 F21 的探针质量，**不要靠放宽断言掩盖**，要改就改成不依赖字面的判据 |
...
1. **F23 prompt 按锚点维护 + 四维度覆盖**：存储形态已定稿（锚点输入 + 卡片/模板版本 + 真实快照），
   接着做覆盖表、来源标注与留痕。
2. **persona 其余字段进 prompt**：background / appearance / speech_style / free_note 现在存了没用
   （F12 只把 identity/background 拼给了解析器，没直接进 prompt）。
3. **I12 修探针判据**（第 71 回那条「招安」不许抠字面）：属 F21，可穿插做。
4. **F21 扩面**（探针补到 12 角色）、**I08 市井线补状态行**：不阻塞主线，可穿插做。
5. **拿试验台找人试**（不是开发任务，但别忘）：让一两个没读过《水浒传》的人第十回和第七十一回
