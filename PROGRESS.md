...
1. **补 F14 的真实模型正向验针**：mock 已测通机制，但"他真认下 → 写新版本"这条端到端还没跑出来
   （两次试探林冲都还在要凭证）。补一条探针，喂够凭证逼出「认下」。
2. **F26 情境选项**（角色提要求时给行动建议）：规格见 DECISIONS 2026-09-16。
   排在 F14 验针之后做，因为「给玉佩 → 他认下 → 关系写入新版本」正好一次验两边。
