...
.PHONY: help setup db db-reset demo admin run test check secrets eval eval-relation eval-summary eval-options status
...
eval-options: ## F26 真实模型验针（跨轮）：选项只在角色要东西时弹，且主语是用户（消耗额度）
	$(PY) scripts/check_action_options.py
